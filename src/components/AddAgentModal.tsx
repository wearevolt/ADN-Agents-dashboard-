...
interface AddAgentModalProps {
  isOpen: boolean
  onClose: () => void
  onAddAgent: (agent: { name: string; apiKey: string; agentUrl: string; webhookUrl: string; description: string; isPrivate: boolean; tags: AgentTag[] }) => void
  onUpdateAgent?: (agentId: string, agent: { name: string; apiKey: string; agentUrl: string; webhookUrl: string; description: string; isPrivate: boolean; tags: AgentTag[] }) => void
  editingAgent?: Agent | null
}

export const AddAgentModal = ({ isOpen, onClose, onAddAgent, onUpdateAgent, editingAgent }: AddAgentModalProps) => {
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    className="sr-only"
                  />
                  <div className={`w-11 h-6 rounded-full transition-colors flex items-center ${isPrivate ? 'bg-blue-600' : 'bg-gray-200'}`}>
                    <div className={`w-5 h-5 rounded-full bg-white shadow-sm transition-transform ${isPrivate ? 'translate-x-5' : 'translate-x-0.5'}`}></div>
                  </div>
                </label>
              </div>
              ...
              <div className="flex space-x-3 pt-4">
                <Button type="button" variant="outline" onClick={onClose} className="flex-1 bg-white border-gray-300 text-gray-900 hover:bg-gray-50">
                  Cancel
                </Button>
